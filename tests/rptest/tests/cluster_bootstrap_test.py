...
class ClusterBootstrapWipedSeedRejoin(RedpandaTest):
    ...
    CLUSTER_DETECTED_EARLY = "detected during early discovery"
    ...
    def test_wiped_seed_rejoin_primes_config_early(self):
        ...
        # The early presence probe should have detected the running cluster and
        # driven the node down the prime path rather than the late resolve path.
        assert self.redpanda.search_log_node(victim, self.CLUSTER_DETECTED_EARLY), (
            "wiped seed did not detect the existing cluster via the early probe"
        )
