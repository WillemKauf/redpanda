...
class NessieCatalogSmokeTest(RedpandaTest):
    ...
    def test_nessie_with_spark(self, cloud_storage_type):
        self.spark = SparkService(self.test_ctx,
                                  self.catalog_service.nessie_url,
                                  self.catalog_service.cloud_storage_warehouse,
                                  self.catalog_service.catalog_type())
        self.spark.start()
        ...
        try:
            ...
            try:
                cursor.execute(
                    "CREATE SCHEMA IF NOT EXISTS `redpanda-iceberg-catalog`.test"
                )
                cursor.execute(
                    "CREATE TABLE IF NOT EXISTS `redpanda-iceberg-catalog`.test.sample_table(id bigint, data string) USING iceberg"
                )
                cursor.execute(
                    "INSERT INTO `redpanda-iceberg-catalog`.test.sample_table VALUES (1, 'Alice'), (2, 'Bob')"
                )
                ...
                cursor.execute(
                    f"CREATE BRANCH IF NOT EXISTS {dev_branch} IN `redpanda-iceberg-catalog`"
                )

                cursor.execute(
                    f"INSERT INTO `redpanda-iceberg-catalog`.test.`sample_table@{dev_branch}` VALUES (3, 'Carol'), (4, 'Doris')"
                )

                cursor.execute(
                    f"SELECT * from `redpanda-iceberg-catalog`.test.`sample_table@{dev_branch}` ORDER BY id"
                )
                row = cursor.fetchall()
                assert len(row) == 4
                ...
                cursor.execute(
                    f"SELECT * from `redpanda-iceberg-catalog`.test.`sample_table@{main_branch}` ORDER BY id"
                )
                row = cursor.fetchall()
                assert len(row) == 2
