...
class EndToEndCloudTopicsBase(EndToEndTest):
    ...
    def wait_until_all_reconciled(
        self,
        topics: Iterable[TopicSpec] | None = None,
        timeout_sec: int = 60,
    ):
...
class EndToEndCloudTopicsLevelingTest(EndToEndCloudTopicsBase):
    """Cloud topics end-to-end test for leveling (rewriting undersized L1 objects)."""
    ...
    def __init__(self, test_context):
        extra_rp_conf = {
            # Start with a small max object size so produces create many
            # small L1 objects.
            "cloud_topics_reconciliation_max_object_size": 1024**2,  # 1 MiB
            # Aggressive leveling scheduling so the test does not have to
            # wait long once the size target is raised.
            "cloud_topics_leveling_interval_ms": 4000,
            # Objects below 50% of the max size are considered undersized.
    ...
    def get_log_levelings(self):
        return self._metric_sum(
            "vectorized_cloud_topics_compaction_scheduler_log_levelings_total"
        )
    ...
    def test_leveling(self):
        ...
        # Produce ~20 MiB of data, creating many small (~1 MiB) L1 objects.
        self.produce()

        # Wait for all data to be reconciled into the metastore. The small
        # max object size produces many objects, so allow extra time over the
        # default wait_until_reconciled.
        self.wait_until_all_reconciled(timeout_sec=120)

        # Raise the max object size so existing objects are now undersized.
        self.rpk.cluster_config_set(
            "cloud_topics_reconciliation_max_object_size",
            83886080,  # 80 MiB
        )

        # Wait for leveling to consolidate the small objects.
        wait_until(
