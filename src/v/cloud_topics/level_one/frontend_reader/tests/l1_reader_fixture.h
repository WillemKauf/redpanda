...
static ss::logger l1_reader_fixture_log("l1_reader_fixture");
...
protected:
    ...
    ss::future<> make_l1_objects(std::vector<tidp_batches_t> batches_by_tidp) {
        ...
        for (auto& [tidp, batches] : batches_by_tidp) {
            term_map[tidp].push_back(l1::metastore::term_offset{
        ...
        std::vector<model::topic_id_partition> tidps;
        tidps.reserve(batches_by_tidp.size());
        for (auto& [tidp, unused] : batches_by_tidp) {
            tidps.push_back(tidp);
        }
        ss::sstring tidps_str = fmt::format("{}", fmt::join(tidps, ","));
        if (!res.has_value()) {
            vlog(
              l1_reader_fixture_log.warn,
              "Failed to produce batches to metastore for tidps: {}, err: {}.",
              tidps_str,
              res.error());
        } else {
            vlog(
              l1_reader_fixture_log.debug,
              "Successfully produced batches to metastore for tidps: {}.",
              tidps_str);
        }
    }
