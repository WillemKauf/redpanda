 ...
 * cloud topics compaction and leveling. It exists on shard0 and manages:
 ...
 * metastore on per-loop sampling intervals.
 * - A `log_compaction_queue` populated with shared pointers to managed logs
 * that require compaction, sorted by a heuristic provided by the
 * `scheduling_policy`. Drained by `compaction_scheduling_loop()`.
 * - A `log_leveling_queue` populated with managed logs that have undersized L1
 * objects to rewrite, sorted by `levelable_bytes` descending. Drained by
 * `leveling_scheduling_loop()`.
 * - A `worker_manager` which manages a `compaction_worker` on each shard,
 * which pull jobs from both queues (compaction has priority). Updates
 * generated by workers are pushed to the `metastore` and cloud storage via
 * multipart uploads.
 */
...
private:
    ...
    ssx::semaphore _sem{
      0, "cloud_topics::compaction_scheduler::compaction_scheduling_loop"};
    ...
    ssx::semaphore _leveling_sem{
      0, "cloud_topics::compaction_scheduler::leveling_scheduling_loop"};
    ...
    // Container of pointers to logs in `_logs/_logs_list` which have sampled
    // leveling metadata available and are available for leveling- i.e
    // `log->leveling_info_and_ts` is guaranteed to have a value and
    // `levelable_bytes > 0`.
