...
ss::future<> compaction_worker::level_range(
  foreign_leveling_job_ptr job, ssx::semaphore_units) {
...
ss::future<> compaction_worker::leveling_work_loop() {
    ...
    while (is_active()) {
        ...
        while (is_active()) {
            auto units_opt = slots.try_get_units(1);
...
ss::future<> compaction_worker::do_level_range(leveling_job* job) {
    ...
    chunked_vector<levelable_range> single_range;
    single_range.push_back(job->range);
