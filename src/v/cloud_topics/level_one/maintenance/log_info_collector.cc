...
ss::future<> log_info_collector::sample_leveling_info(
  chunked_vector<log_compaction_meta_ptr> logs) const {
    ...
    auto threshold
      = config::shard_local_cfg().cloud_topics_leveling_object_size_threshold();
    auto min_acceptable = static_cast<size_t>(
      static_cast<double>(target_size) * threshold);
    ...
    for (const auto& log : logs) {
        ...
        log->leveling_info_and_ts = leveling_info_and_timestamp{
          .info = result.value(), .collected_at = now};
