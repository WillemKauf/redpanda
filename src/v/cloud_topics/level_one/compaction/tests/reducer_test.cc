...
//     auto sink = std::make_unique<l1::in_memory_sink>(test_tidp,
//     &output_objs); auto reducer = compaction::sliding_window_reducer(
//       std::move(src), std::move(sink));
//
...
// public:
//     l1::file_and_md_info take_updates_from_committer(
//       l1::compaction_committer& committer, model::topic_id_partition tidp) {
//         auto& job_state = committer._compaction_jobs.at(tidp);
//         auto updates = std::exchange(job_state->staging_file_and_md_infos,
//         {}); return updates;
//     }
//
...
//       std::move(output_batches));
//
//     // TODO: once we hook up the committer to the metastore, it would be nice
//     to
...
//         _metastore.get_compaction_info(info_spec).get();
//
//         ASSERT_TRUE(compaction_info.has_value());
//         ASSERT_FLOAT_EQ(compaction_info->dirty_ratio, 1.0);
//         ASSERT_TRUE(compaction_info->offsets_response.dirty_ranges.covers(
...
//         ASSERT_EQ(
//           update.compact_update.new_cleaned_range->base_offset,
//           start_offset);
//         ASSERT_EQ(
...
//         auto oid = l1::create_object_id();
//         ASSERT_TRUE(
//           _io.put_object(oid, file_and_md_info.staging_file.get(),
//           &as).get());
//         auto object_stream
//           = _io
//               .read_object(
//                 l1::object_extent{.id = oid, .position = 0, .size = size},
//                 &as)
//               .get();
//         ASSERT_TRUE(object_stream.has_value());
//
