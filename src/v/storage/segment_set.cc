    ...
    return ss::async([segments = std::move(segments),
        ...
        for (auto& s : to_recover) {
            ...
            if (term_parser) {
                ...
                // replay visited every batch with the parser in hand, so
                // it re-derived whether all configuration batches carry
                // their term (e.g. re-proving stamped segments after index
                // loss). Replay cannot stamp, so an unproven segment is
                // left for a compaction rewrite to settle.
                s->index().set_config_batch_terms_verified(
                  !recovered.saw_configuration_without_term);
            }
            // persist index
            s->index().flush().get();
