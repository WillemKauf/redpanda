...
///
/// v1: Record batch headers are serialized to disk through the old hand-rolled
///     ADL code. The segment contains batches from exactly one raft term (the
///     term in the filename).
/// v2: Record batch headers are serialized to disk in the v1 layout with the
///     batch's raft term appended. The segment may contain batches from
///     multiple raft terms and the {term} in the filename is the term of the
///     base offset only.
