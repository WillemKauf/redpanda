...
public:
    ...
    struct checkpoint {
        ...
        chunked_vector<std::pair<model::term_id, model::offset>>
          term_transitions;
        // whether any raft configuration batch passed through without a
        // term in its payload (or without a parser to check); replay visits
        // every batch, so a false value proves the whole segment is stamped
        bool saw_configuration_without_term{false};
