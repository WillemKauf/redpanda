...
struct linear_int_kv_batch_generator {
    ...
    // ran correctly.
    static void
    validate_post_compaction(model::record_batch_reader::data_t&& batches) {
        int idx = 0;
...
public:
    ...
    model::record_batch_reader::data_t read_and_validate_all_batches(
    ...
    template<typename T = random_batches_generator>
        requires requires(T generator, std::optional<model::timestamp> ts) {
            { generator(ts) } -> std::same_as<model::record_batch_reader::data_t>;
        }
    // clang-format on
    ...
    // inclusive
    model::record_batch_reader::data_t read_range_to_vector(
      ss::shared_ptr<storage::log> log,
      model::offset start,
      model::offset end) {
        storage::log_reader_config cfg(
