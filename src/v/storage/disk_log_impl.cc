...
ss::future<> disk_log_impl::load_compaction_state() {
    ...
    auto value = _kvstore.get(
      ...
      internal::compaction_state_key(config().ntp()));
    if (value) {
        _compaction_state
          = std::make_unique<local_compaction::compaction_state>(
            serde::from_iobuf<local_compaction::compaction_state>(
              std::move(*value)));
    } else {
        _compaction_state
          = std::make_unique<local_compaction::compaction_state>();
...
std::optional<iobuf> disk_log_impl::serialize_compaction_state() const {
    if (!_compaction_state) {
        return std::nullopt;
    }
    return serde::to_iobuf(*_compaction_state);
}

ss::future<> disk_log_impl::apply_storage_metadata(iobuf buf) {
    _compaction_state = std::make_unique<local_compaction::compaction_state>(
      serde::from_iobuf<local_compaction::compaction_state>(std::move(buf)));
    co_await _kvstore.put(
      kvstore::key_space::storage,
      internal::compaction_state_key(config().ntp()),
      serde::to_iobuf(*_compaction_state));
}
...
ss::future<> disk_log_impl::do_compact(
  ...
  std::optional<model::offset> new_start_offset) {
    ...
    if (_compaction_state) {
        local_compaction::compaction_worker worker;
        co_await worker.compact(*this, *_compaction_state, compact_cfg);
        co_return;
    }
...
ss::future<> disk_log_impl::copy_kvstore_state(
  ...
  ss::sharded<storage::api>& storage) {
    ...
    std::optional<iobuf> compaction_st = source_kvs.get(
      ks, internal::compaction_state_key(ntp));
    ...
    co_await storage.invoke_on(target_shard, [&](storage::api& api) {
        ...
        if (compaction_st) {
            write_futures.push_back(api.kvs().put(
              ks, internal::compaction_state_key(ntp), compaction_st->copy()));
        }
...
ss::future<> disk_log_impl::remove_kvstore_state(
  const model::ntp& ntp, storage::kvstore& kvs) {
    ...
    return ss::when_all_succeed(
             ...
             kvs.remove(ks, internal::compaction_state_key(ntp)))
      .discard_result();
}
