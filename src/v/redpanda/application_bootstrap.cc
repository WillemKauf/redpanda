...
application::register_and_apply_join_snapshot(bool defer_needs_restart) {
...
bool application::should_prime_identity_early() {
    // Only a first-time (unregistered) node primes its identity early.
    // Established and overridden nodes resolve later in
    // resolve_node_identity().
    if (_node_id_source != node_id_source::unregistered) {
        return false;
    }

    // A non-seed joiner always learns the cluster's configuration from its join
    // reply, so it can and should prime early.
    if (!is_seed_node()) {
        return true;
    }
    // An unregistered seed is either a genuine founder or a wiped seed
    // rejoining an existing cluster - try to detect one here.
    static constexpr auto seed_presence_probe_budget = std::chrono::seconds(2);
    return _cluster_discovery
      ->try_detect_existing_cluster(seed_presence_probe_budget)
      .get();
}

ss::future<> application::prime_node_identity() {
    auto node_id = co_await register_and_apply_join_snapshot();
...
ss::future<> application::resolve_node_identity() {
    ...
    case node_id_source::unregistered:
        ...
        // seed rejoining whose early presence probe was inconclusive). This
        // runs after initialization has has already occured, so apply the join
        // snapshot with needs_restart configs pending.
        node_id = co_await register_and_apply_join_snapshot(
          /*defer_needs_restart=*/true);
        break;
    }
...
ss::future<> application::apply_controller_snapshot(
  const cluster::controller_join_snapshot& snap, bool defer_needs_restart) {
