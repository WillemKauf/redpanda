...
private:
    ...
    // Registers with the cluster and, if a controller_join_snapshot is returned
    // in the join reply, applies it and returns the assigned node ID. When
    // defer_needs_restart is true, needs_restart configs are applied as pending
    // rather than promoted live.
    ss::future<model::node_id>
    register_and_apply_join_snapshot(bool defer_needs_restart = false);
    // Resolves node identity early by registering with the cluster and applying
    // the join snapshot. Used for a first-time joiner (either a non-seed, or a
    // seed that detected an existing cluster).
    ss::future<> prime_node_identity();
    // True if this node should resolve its identity early. Returns true for
    // joining nodes which are as of yet unregistered (do not have a persisted
    // node_id). For seed nodes (which may be forming a cluster for the first
    // time, or may be a first-time joiner from a wiped seed-node), we attempt
    // to detect whether a cluster is already established or not. If a cluster
    // is already established, this seed node can simply take the fast joiner
    // path. Otherwise, defer identity resolution.
    bool should_prime_identity_early();
