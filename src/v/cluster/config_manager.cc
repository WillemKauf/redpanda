  ...
  bool defer_needs_restart = false) {
    ...
    if (cfg.contains(key)) {
        ...
        try {
            ...
            bool set = (defer_needs_restart && property.needs_restart())
...
ss::future<config_manager::preload_result> config_manager::preload_join(
  const controller_join_snapshot& snap, bool defer_needs_restart) {
    ...
    for (const auto& i : snap.config.values) {
        ...
        preload_local(key, value, std::ref(result), defer_needs_restart);

        // Broadcast value to all shards
        co_await ss::smp::invoke_on_all([&key, &value, defer_needs_restart]() {
            preload_local(key, value, std::nullopt, defer_needs_restart);
        });
    }
