...
scan_remote_partition_incrementally_with_reuploads(
  ...
  size_t maybe_max_readers = 0) {
    ...
    auto manifest = hydrate_manifest(
      fixt.api.local(), cloud_storage_clients::bucket_name{fixt.bucket_name});
    partition_probe probe(manifest.get_ntp());
    auto manifest_view = ss::make_shared<async_manifest_view>(
      fixt.api,
      fixt.cache,
      manifest,
      cloud_storage_clients::bucket_name{fixt.bucket_name});
...
FIXTURE_TEST(test_scan_while_shutting_down, cloud_storage_fixture) {
    ...
    auto manifest = hydrate_manifest(
      api.local(), cloud_storage_clients::bucket_name{bucket_name});
    partition_probe probe(manifest.get_ntp());
    auto manifest_view = ss::make_shared<async_manifest_view>(
      api, cache, manifest, cloud_storage_clients::bucket_name{bucket_name});
    auto partition = ss::make_shared<remote_partition>(
      manifest_view,
      api.local(),
      this->cache.local(),
