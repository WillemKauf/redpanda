...
using compaction_offset_interval_set
  = container::offset_interval_set<OffsetT>;

/// \brief Dirty and tombstone-removable offset ranges for a partition.
template<typename OffsetT>
...
/// \brief Tracks which offset ranges of a log have been compacted,
/// and which contain tombstones eligible for removal after
...
struct compaction_state
  : serde::envelope<
    ...
    /// \brief A cleaned offset range that contained tombstones at the time
    /// of cleaning. The timestamp is used to determine when tombstones in
    /// this range become eligible for removal.
    ...
    friend bool
    operator==(const compaction_state&, const compaction_state&)
      = default;
    auto serde_fields() {
    ...
    /// \brief Returns false if the input range overlaps with another
    /// existing range with tombstones.
    bool may_add(const cleaned_range_with_tombstones&) const;

    /// \brief Adds the input range to the set of cleaned ranges with
    /// tombstones. Returns false if may_add() returns false.
    bool add(const cleaned_range_with_tombstones&);

    /// \brief Returns true if the input inclusive range is fully covered by
    /// a contiguous set of cleaned ranges with tombstones.
    bool has_contiguous_range_with_tombstones(OffsetT, OffsetT) const;

    /// \brief Removes the input inclusive range from the set of cleaned
    /// ranges with tombstones. The input range doesn't need to align exactly
    /// with any of cleaned_ranges_with_tombstones, but it must be fully
    /// covered by a contiguous set of ranges. Returns false if no such
    /// contiguous set exists.
    bool erase_contiguous_range_with_tombstones(OffsetT, OffsetT);

    /// \brief Prefix truncates cleaned_ranges and
    /// cleaned_ranges_with_tombstones so that all ranges below the new start
    /// are removed, and any range that overlaps with the new start is
    /// truncated to start at the given offset.
    void truncate_with_new_start_offset(OffsetT);

    /// \brief Computes dirty and tombstone-removable ranges.
    ///
    ...
    // Ranges of the log whose keys have been deduplicated from the beginning
    // of the log to and including the interval's last offset.
    ...
    //
    // For a tombstone record to be eligible for removal, all offsets at and
...
private:
    ...
    // Returns iterators that span the contiguous, minimal set that fully
    // covers the given inclusive offset range. If no such set of contiguous
    // ranges exists, returns std::nullopt.
